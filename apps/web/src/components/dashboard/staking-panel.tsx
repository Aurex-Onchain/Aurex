  ...
  if (!isConnected) {
    return (
      <div className="p-6 rounded-lg border border-zinc-800 bg-zinc-900/50 text-center">
        <p className="text-zinc-400">{t("staking.connectWallet")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 rounded-lg border border-zinc-800 bg-zinc-900/50 space-y-4">
      <div className="flex items-center gap-2">
        <span className="material-icons-outlined text-indigo-400 text-xl">token</span>
      ...
      {!isRegistered ? (
        ...
        <div className="space-y-3">
          <p className="text-sm text-zinc-300">{t("staking.becomePublisher")}</p>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              placeholder="100"
              className="w-40 px-3 py-2 rounded bg-zinc-800 border border-zinc-700 text-sm text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-indigo-500"
            />
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-sm text-zinc-300">{t("staking.increaseStake")}</p>
            <div className="flex items-center gap-3">
              <input
                type="number"
                min="1"
          ...
          <div className="pt-2 border-t border-zinc-800 space-y-2">
            ...
            <button
              ...
              disabled={isBusy}
              className="flex items-center gap-2 px-4 py-2 rounded bg-zinc-800 border border-zinc-700 text-red-400 text-sm font-medium hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
      ...
      {(stakeSuccess || unregisterSuccess) && (
        <div className="p-3 rounded border border-emerald-900/50 bg-emerald-950/30 text-sm text-emerald-400 flex items-center gap-2">
      ...
      {txError && (
        <div className="p-3 rounded border border-red-900/50 bg-red-950/30 text-sm text-red-400">
          {txError.message.slice(0, 200)}
          <button
            onClick={() => { resetApprove(); resetStake(); resetUnregister(); }}
            className="ml-2 underline text-xs text-red-500 hover:text-red-300"
          >
            {t("staking.dismiss")}
          </button>
        </div>

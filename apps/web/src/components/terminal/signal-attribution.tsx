...
export function SignalAttribution({ signal, policy }: Props) {
  ...
  return (
    <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-500/20 space-y-2.5">
      {/* Header badge */}
      <div className="flex items-center gap-2">
        <span className="material-icons-outlined text-sm text-emerald-400">auto_awesome</span>
        <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-300 uppercase tracking-wide">
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="font-mono text-zinc-800 dark:text-zinc-200">{formatAddress(signal.signer)}</span>
      </div>

      {/* Fee adjustment row */}
      <div className="flex items-center justify-between text-xs">
        <span className="text-zinc-600 dark:text-zinc-400">{t("terminal.feeAdjustment")}</span>
        <div className="flex items-center gap-1.5">
          <span className="text-zinc-500">{t("terminal.feeDefault")} {formatFee(defaultFee)}</span>
          <span className="material-icons-outlined text-sm text-emerald-500 dark:text-emerald-400">arrow_forward</span>
          <span className="text-emerald-600 dark:text-emerald-300 font-medium">
            {t("terminal.feeDynamic")} {formatFee(dynamicFee)}
          </span>
          <span className="px-1.5 py-0.5 rounded bg-emerald-100 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/25 text-emerald-600 dark:text-emerald-300 text-[10px] font-medium">

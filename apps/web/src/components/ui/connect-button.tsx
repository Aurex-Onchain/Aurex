...
export function ConnectButton() {
  ...
  if (isConnected && address) {
    return (
      <div className="w-full flex items-center gap-2 px-3 py-2 rounded-md bg-zinc-800">
        ...
        >
          ...
          <span className="block text-sm font-mono text-zinc-300 truncate">{formatAddress(address)}</span>
        </button>
      </div>
    );
  }

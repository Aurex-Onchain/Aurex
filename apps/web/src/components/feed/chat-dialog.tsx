      ...
      <div className="relative w-full max-w-lg mx-4 mb-4 sm:mb-0 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl flex flex-col max-h-[70vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-2">
            <span className="material-icons-outlined text-emerald-400 text-lg">smart_toy</span>
            <span className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{t("chat.title")}</span>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-zinc-100 dark:bg-zinc-800 text-zinc-400 px-4 py-2.5 rounded-2xl rounded-bl-md text-sm">
        ...
        {/* Input */}
        <form onSubmit={handleSend} className="px-5 py-4 border-t border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            <input
              ...
              disabled={loading}
              className="flex-1 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-zinc-800 dark:text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-emerald-500 disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="w-9 h-9 rounded-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white flex items-center justify-center transition-colors"
            >

...
function ChatInputBar({ onSend }: { onSend: (content: string) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="backdrop-blur-md bg-zinc-900/70 border border-zinc-800 rounded-xl p-4 shadow-lg">
      <input
        type="text"
        value={value}
      ...
      <div className="flex justify-end mt-3">
        <button
          type="submit"
          disabled={!value.trim()}
          className="w-9 h-9 rounded-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white flex items-center justify-center transition-colors"
        >
